...
import com.example.aegis_be.domain.eventlog.dto.EventLogCreateRequest;
...
public class EventLogController {
    ...
    @Operation(
            summary = "사용자 텍스트 입력 로그 저장",
            description = """
                    사용자가 직접 입력한 텍스트를 이벤트 로그에 즉시 저장합니다.

                    **호출 주체**: 프론트엔드

                    **저장 형식**: "사용자 입력: {입력한 텍스트}" 형태로 description에 저장됩니다.
    ...
    @ApiResponses({
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "200",
                    description = "저장 성공 - 생성된 이벤트 로그 반환"
            ),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "400",
                    description = "입력 텍스트 누락 - description이 비어있음"
            ),
            @io.swagger.v3.oas.annotations.responses.ApiResponse(
                    responseCode = "401",
                    description = "인증 실패 - 유효하지 않은 Access Token"
    ...
    @PostMapping
    public ApiResponse<EventLogResponse> createUserInputLog(
            @AuthenticationPrincipal CustomUserDetails userDetails,
            @Parameter(description = "출동 세션 ID") @PathVariable Long sessionId,
            @Valid @RequestBody EventLogCreateRequest request) {
        return ApiResponse.success(
                eventLogService.createUserInputLog(userDetails.getName(), sessionId, request.getDescription()));
    }
}
